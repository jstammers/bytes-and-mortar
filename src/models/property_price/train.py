...
def _load_data(experiment: Experiment) -> pl.DataFrame:
    """Load processed parquet, joining regional HPI data when needed.

    If any required feature columns are absent from the sales parquet
    (e.g. ``averageprice``, ``index`` after storage separation), the function
    automatically joins ``uk_hpi_regional.parquet`` on (district, year, month)
    before selecting the required columns.
    """
    ...
    sales_lf = pl.scan_parquet(experiment.data_path)
    sales_schema = set(sales_lf.collect_schema().names())

    # If any required columns are missing from the sales parquet, try to join
    # the regional HPI parquet (generated by `run_pipeline` when HPI is provided).
    missing_cols = [c for c in required_cols if c not in sales_schema]
    if missing_cols:
        from src.data.config import HPI_REGIONAL_DATA_PATH

        logger.info(
            "Columns %s not in sales parquet; joining HPI regional data from %s",
            missing_cols,
            HPI_REGIONAL_DATA_PATH,
        )
        sales_lf = join_hpi_to_sales(sales_lf, HPI_REGIONAL_DATA_PATH)

    available = set(sales_lf.collect_schema().names())
    select_cols = [c for c in required_cols if c in available]
    lazy_df = sales_lf.select(select_cols)
    if experiment.nrows:
