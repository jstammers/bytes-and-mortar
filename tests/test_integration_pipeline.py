"""Integration tests for the full pipeline using postcode fixture data.

These tests run end-to-end on a real-data subset to verify correctness and
streaming memory behaviour.

Fixture files must be present in data/test_fixtures/raw/ before running.
Generate them with:
    uv run python scripts/create_fixtures.py --postcode-prefix <PREFIX>

The postcode prefix used during fixture generation is read automatically from
data/test_fixtures/raw/postcode_prefix.txt (written by create_fixtures.py).
...
def _read_postcode_prefix() -> str:
    """Read the postcode prefix from the fixture metadata file.

    Falls back to 'W1' if the metadata file is absent (e.g. fixtures were
    generated by the old create_m19_fixtures.py script).  Regenerate fixtures
    with create_fixtures.py to produce the metadata file.
    """
    prefix_file = FIXTURE_RAW_DIR / "postcode_prefix.txt"
    if prefix_file.exists():
        return prefix_file.read_text().strip()
    return "W1"


POSTCODE_PREFIX = _read_postcode_prefix()
...
def _fixture_dir() -> Path:
    ...
    if not FIXTURE_RAW_DIR.exists():
        pytest.skip(
            ...
            "Run: uv run python scripts/create_fixtures.py --postcode-prefix <PREFIX>"
        )
...
def sales_lf(fixture_raw_dir: Path) -> pl.LazyFrame:
    """Cleaned Land Registry LazyFrame from the integration fixture."""
...
def epc_lf(fixture_raw_dir: Path) -> pl.LazyFrame:
    """Cleaned EPC LazyFrame from the integration fixture."""
...
def hpi_lf(fixture_raw_dir: Path) -> pl.LazyFrame:
    """Cleaned UK HPI LazyFrame from the integration fixture."""
...
class TestSourceLoading:
    ...
    def test_land_registry_all_expected_postcodes(self, sales_lf: pl.LazyFrame) -> None:
        """Every cleaned LR row must have a postcode starting with the fixture prefix."""
        df = sales_lf.collect()
        non_prefix = df.filter(~pl.col("postcode").str.starts_with(POSTCODE_PREFIX))
        assert len(non_prefix) == 0, (
            f"{len(non_prefix)} rows have non-{POSTCODE_PREFIX} postcodes after cleaning: "
            f"{non_prefix['postcode'].unique().to_list()[:5]}"
        )
    ...
    def test_epc_all_expected_postcodes(self, epc_lf: pl.LazyFrame) -> None:
        """Every cleaned EPC row must have a postcode starting with the fixture prefix."""
        df = epc_lf.collect()
        non_prefix = df.filter(~pl.col("postcode").str.starts_with(POSTCODE_PREFIX))
        assert len(non_prefix) == 0, (
            f"{len(non_prefix)} EPC rows have non-{POSTCODE_PREFIX} postcodes: "
            f"{non_prefix['postcode'].unique().to_list()[:5]}"
        )
...
class TestPipelineIntegration:
    def test_pipeline_sales_only(self, sales_lf: pl.LazyFrame, tmp_path: Path) -> None:
        ...
        result = run_pipeline(
            sales=sales_lf,
            output_name="integration_sales_only",
            output_dir=tmp_path,
            fmt="parquet",
        )
        assert isinstance(result, pl.DataFrame)
        assert len(result) > 0
        assert (tmp_path / "integration_sales_only.parquet").exists()
    ...
    def test_pipeline_epc_join_has_matches(
        ...
        """At least 10% of sales should match an EPC record by postcode."""
        result = run_pipeline(
            sales=sales_lf,
            epc=epc_lf,
            output_name="integration_epc_matches",
            output_dir=tmp_path,
            fmt="parquet",
        )
        ...
        assert match_rate > 0.1, (
            f"Only {match_rate:.1%} of {POSTCODE_PREFIX} sales matched an EPC record. "
            "Expected >10%. Check the postcode join logic."
        )

    def test_pipeline_output_postcodes_match_prefix(
        self, sales_lf: pl.LazyFrame, epc_lf: pl.LazyFrame, tmp_path: Path
    ) -> None:
        """No unexpected postcodes should appear in the pipeline output."""
        result = run_pipeline(
            sales=sales_lf,
            epc=epc_lf,
            output_name="integration_postcodes",
            output_dir=tmp_path,
            fmt="parquet",
        )
        non_prefix = result.filter(~pl.col("postcode").str.starts_with(POSTCODE_PREFIX))
        assert len(non_prefix) == 0, (
            f"{len(non_prefix)} output rows have non-{POSTCODE_PREFIX} postcodes — "
    ...
    def test_pipeline_full_with_hpi(
        ...
        result = run_pipeline(
            ...
            epc=epc_lf,
            hpi=hpi_lf,
            output_name="integration_full",
            output_dir=tmp_path,
            fmt="parquet",
        )
        assert isinstance(result, pl.DataFrame)
        assert len(result) > 0
        assert (tmp_path / "integration_full.parquet").exists()
    ...
    def test_pipeline_output_is_parquet(self, sales_lf: pl.LazyFrame, tmp_path: Path) -> None:
        ...
        run_pipeline(
            sales=sales_lf,
            output_name="integration_roundtrip",
            output_dir=tmp_path,
            fmt="parquet",
        )
        dest = tmp_path / "integration_roundtrip.parquet"
        assert dest.exists()
